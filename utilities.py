...
def load_lemurs_data(filename, i_idcs=None):
    """Load showers and incident energies from a LEMURS HDF5 file.

    Returns showers (K, R, P, N) and incident_energies (K,) in MeV.
    """
    with h5py.File(filename, "r") as f:
        ...
        incident_energies = np.array(incident_energies).flatten()
    print("Showers shape:", showers.shape)
    print("Incident energies shape:", incident_energies.shape)
    return showers, incident_energies


def load_calochallenge_data(filename, hlf, i_idcs=None):
    """Load flat CaloChallenge showers and reshape to (K, R, PHI, Z) using HLF binning.
    ...
    Returns showers_4d (K, max_R, max_PHI, num_layers) and incident_energies (K, 1).
    """
    with h5py.File(filename, "r") as f:
        print("Features in " + os.path.basename(filename) + ":", list(f.keys()))
        showers = f["showers"][i_idcs, :] if i_idcs is not None else f["showers"][:]
    ...
    return showers_4d, incident_energies
